...
const useSearchStore = create<SearchState>()(
  devtools((set, get) => ({
    ...
    clearResults: () => set((state) => ({ searchResults: (state.searchResults = []), offset: 0, hasMore: true })),

    customSearch: async (searchTerm) => {
      set((state) => ({ noResults: false }));
      try {
        const response = await fetch(
          `https://api.giphy.com/v1/gifs/search?q=${searchTerm}&api_key=${get().key}&limit=${get().fetchAmount}&offset=${get().offset+1}`
        );
    ...
    trendingSearch: async () => {
      try {
        const response = await fetch(
          `https://api.giphy.com/v1/gifs/trending?api_key=${get().key}&limit=${get().fetchAmount}&offset=${get().offset+1}`
        );
        const results = await response.json();
        get().addResults(results.data);
        set((state) => ({ offset: state.offset + get().fetchAmount }));
    ...
    randomSearch: async () => {
      try {
        const response = await fetch(
          `https://api.giphy.com/v1/gifs/random?api_key=${get().key}`
        );
